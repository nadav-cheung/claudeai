...
// 从 buffer 解析完整的 SSE 事件(事件之间以空行 \n\n 分隔)
function parseSSEBuffer(buffer: string): { event: string; data: string }[] {
  const events: { event: string; data: string }[] = []
  // 按空行切分;末尾未跟空行的段是不完整事件,留给 getRemainingBuffer 保留
...
// 返回 buffer 中最后一个完整事件之后的剩余(未收完的)部分
...
在 StreamParser 中，这个机制天然生效——你 `yield` 每个 delta，消费者（Agentic Loop）处理完当前 delta 后，流才继续读取下一块。
