...
这个问题看起来简单——翻开 `src/` 看到的主要是 `.ts` 和 `.tsx` 文件，1884 个，仅有极少数 `.js` 脚本（如部分命令脚本）作为例外。但如果在 2024 年做一个 AI 编程助手的 CLI 工具，摆在桌面上的选项远不止 TypeScript 一个。Python 是 AI 领域的霸主，Rust 有性能和安全，Go 有简洁和速度，甚至连纯 JavaScript 都能说一句"我更简单"。
...
`src/` 目录下几乎没有 `.js` 文件（仅个别命令脚本）。TypeScript 是硬性要求。编译器是守门人，类型系统是契约，没有通过类型检查的代码进不了代码库。
