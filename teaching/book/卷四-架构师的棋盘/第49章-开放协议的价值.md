...
优先级从低到高。claude.ai 网页端的连接器优先级最低（被以下所有来源覆盖）；用户在 `.mcp.json` 里配的 project 级服务器优先于 claude.ai；企业管理员配的 enterprise 级配置绝对独占（存在时直接接管，不加载 claude.ai）。这不是随意的排序——它反映了控制权的主张：enterprise（独占） > local > project > user > plugin > claudeai。
...
- **六层配置作用域**：claudeai -> plugin -> user -> project -> local -> enterprise
