...
```typescript
// → src/services/tools/toolExecution.ts（内联构造 tool_result，无独立命名函数）
// 工具执行后，结果被内联构造成 tool_result content block 回传给模型
...
长时间运行的工具（如 `Bash` 执行 `npm install`）由各自工具内部管理超时——例如 `Bash` 工具对命令执行有自己的超时机制。工具执行层本身并不用统一的 `withTimeout` 包裹所有 `tool.call()`。超时或失败的结果被标记为 `is_error: true` 返回给模型，模型可以决定重试或换一种方式。
...
7. **结果格式化**：原始输出被内联构造成 API 要求的 tool_result 格式（无独立命名函数），`is_error` 让模型看到失败
8. **取消传播**：全局 `abortController`——一次 Ctrl+C 终止全部工具
9. **超时保护**：由各工具内部管理（如 Bash 命令超时），工具执行层本身无统一超时常量；失败标记 is_error 返回模型
