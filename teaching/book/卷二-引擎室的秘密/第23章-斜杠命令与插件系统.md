...
type CommandBase = {
  ...
  availability?: CommandAvailability[]  // 可用性要求（取值 'claude-ai' | 'console'，可多选）
...
> 注：`supportsNonInteractive?: boolean` 实际定义在 `LocalCommand` 上（非 `CommandBase`）。

`availability` 字段控制命令的可见性——它是一个数组，取值 `'claude-ai'` 或 `'console'`，决定命令在哪些客户端（如 claude.ai 网站或终端 console）可见；不设则到处可见。`isEnabled` 是运行时检查——比如 `daemon` 命令只在 daemon 功能开启时才出现。
...
[上一章：安全门卫](./第22章-安全门卫.md) | [下一章：Hook系统](./第24章-Hook系统.md)
