...
1. **claude.ai 配置**（`scope: 'claudeai'`）——从 claude.ai 网站同步（优先级最低，被以下来源覆盖）
2. **插件配置**（`scope: 'plugin'`）——来自已安装的插件
3. **用户配置**（`scope: 'user'`）——来自全局 `settings.json`，跨项目生效
4. **项目配置**（`scope: 'project'`）——来自项目根目录的 `.mcp.json`
5. **本地配置**（`scope: 'local'`）——来自 `settings.local.json`，不提交到版本控制
6. **企业配置**（`scope: 'enterprise'`）——管理员分发，绝对独占（存在时直接接管，不加载 claude.ai）
...
[上一章：Hook系统](./第24章-Hook系统.md) | [下一章：Agent的克隆与协作](./第26章-Agent的克隆与协作.md)
