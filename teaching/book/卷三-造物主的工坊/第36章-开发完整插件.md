...
Claude Code 定义了 27 种 HookEvent，覆盖了会话、工具、权限、协作等各个阶段：
...
这些事件在 `src/entrypoints/sdk/coreTypes.ts` 的 `HOOK_EVENTS` 数组中定义（由 `src/schemas/hooks.ts` 导入使用）。绝大多数插件只需要 `PreToolUse` 和 `PostToolUse`。
...
[上一章：构建多 Agent 协作](./第35章-构建多Agent协作.md) | [下一章：编写测试](./第37章-编写测试.md)
