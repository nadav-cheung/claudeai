...
`src/` 目录下的结构，你在第 17 章（打开引擎室的门）已经见过了。快速回忆一下关键位置：
...
| 文件 | 作用 | 你在哪一章认识的 |
|------|------|--------------|
| `src/entrypoints/cli.tsx` | 最先执行的代码，处理 `--version`、`--daemon-worker` 等快速路径 | 第 18 章 |
| `src/entrypoints/init.ts` | 配置、网络、代理、遥测的初始化 | 第 18 章 |
| `src/main.tsx` | Commander 命令定义、REPL 启动 | 第 3 章 |
| `src/tools.ts` | 工具注册表，`getAllBaseTools()` | 第 20 章 |
...
2. **在 `cli.js` 中搜索 `profileCheckpoint`**，数一数有多少条启动路径。对比你在第 18 章学到的启动流程图。
3. **打开 `src/tools/` 目录**，浏览工具列表。你在第 20 章学过哪些工具？看看它们的目录结构是否如你所料。
...
[上一章：你已经是引擎师了](../卷二-引擎室的秘密/第28章-API通信的暗面.md) | [下一章：第一次修改](./第30章-第一次修改.md)
