...
**提示**：
- 使用 commander.js 的 `.option()` 添加标志
- 根据标志控制 console.log 的输出
...
```typescript
const themes = {
  light: { error: 'red', warn: 'yellow', info: 'blue' },
  dark: { error: 'brightRed', warn: 'brightYellow', info: 'brightBlue' }
};

function getTheme() {
  return process.argv.includes('--dark') ? themes.dark : themes.light;
}
```

### 练习 3：实现交互式选择菜单

**目标**：用 inquirer 实现下拉式选择菜单

**提示**：
- 使用 `inquirer.prompt([{ type: 'list', ... }])`
- 处理用户选择

**答案要点**：
```typescript
async function selectFromList(items: string[], message: string) {
...
👉 [01 - 构建你的第一个CLI工具](01-构建你的第一个CLI工具.md) —— 开始实战项目
