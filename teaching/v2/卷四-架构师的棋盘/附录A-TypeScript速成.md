---
title: "附录A — TypeScript 速成"
description: "从基础类型到高级模式，浓缩 TypeScript 核心知识，帮助读者快速掌握本书所需的类型系统能力。"
tags: [typescript, generics, types, async, patterns]
date: 2026-05-10
---

# 附录A — TypeScript 速成

> 本附录浓缩 TypeScript 核心知识：基础类型、接口、泛型、联合/交叉类型、async/await，以及 Branded Types、Discriminated Unions、TypedEventEmitter、DeepPartial 等高级模式。
...
接口支持继承与方法定义，是 TypeScript 表达对象契约的主要手段。
...
function processValue(value: string | number) {
  if (typeof value === "string") {
    console.log(value.toUpperCase());  // 这里 value 是 string
  } else {
...
emitter.on('message', (data) => {
  console.log(`${data.user}: ${data.text}`);  // data 类型自动推断
});
...
updateConfig({
  api: { url: 'http://localhost:3000' }  // 只改 url，timeout 和 retries 保持不变
});
```
...
| 概念 | 关键词 |
|------|--------|
| 接口与工具类型 | `interface`、`Partial`、`Pick`、`Omit`、`Record` |
| 联合与交叉 | `A \| B`、`A & B`、类型守卫 |
| 泛型 | `<T>`、`extends` 约束、条件类型、映射类型 |
| 异步编程 | `async/await`、`Promise.all`、`AsyncGenerator` |
| Branded Types | `string & { __brand }` 防止语义混淆 |
| Discriminated Unions | 公共 `type` 字段 + `switch` 穷尽检查 |
| TypedEventEmitter | 泛型 `EventMap` 确保事件名与数据类型对应 |
| DeepPartial | 递归可选，适用于嵌套配置的部分更新 |
